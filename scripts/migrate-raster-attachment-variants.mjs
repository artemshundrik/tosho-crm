...
import { renderRasterPreviewFiles } from "./attachment-preview-renderer.mjs";
...
const matchFilter = (process.env.RASTER_ATTACHMENT_MIGRATION_MATCH || "").trim().toLowerCase();
const migrationLimit = Number(process.env.RASTER_ATTACHMENT_MIGRATION_LIMIT || "0");
...
async function migrateOne({ bucket, storagePath }) {
  ...
  const tempDir = await fs.mkdtemp(path.join(os.tmpdir(), "attachment-raster-migrate-"));
