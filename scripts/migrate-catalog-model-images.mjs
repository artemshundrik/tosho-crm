/**
 * Переїзд фото моделей каталогу з чужих сайтів у наше сховище.
 *
 * НАВІЩО, КОЛИ ЧУЖЕ ПОСИЛАННЯ ТЕЖ ПОКАЗУЄТЬСЯ. Показується воно лише в HTML:
 * `<img>` CORS не питає. А все інше — так. 21.09.2026 на цьому впав PDF
 * комерційної пропозиції: щоб покласти фото в документ, його треба ПРОЧИТАТИ
 * (`fetch` → полотно), а eney.com.ua, avanprint.ua, totobi.com.ua і
 * papirus-opt.com читати себе не дають. Позиція мовчки лишалась без знімка.
 * Плюс чуже посилання вмирає разом із чужою сторінкою, а ми на нього посилаємось
 * у документі, який уже поїхав замовнику.
 *
 * ЯК ЗАПУСКАТИ. Спершу холостий хід — він усе качає й показує, що збирається
 * зробити, але нічого не пише:
 *
 *   npm run migrate:catalog-images
 *
 * Потім по-справжньому:
 *
 *   CATALOG_IMAGE_MIGRATION_DRY_RUN=0 npm run migrate:catalog-images
 *
 * Ручки: CATALOG_IMAGE_MIGRATION_LIMIT, _MATCH (підрядок назви), _TEAM_ID,
 * _ONLY_EXTERNAL=0 (перемолоти й наші теж).
 *
 * ПРОГОНИ. 21.09.2026 — 10 моделей (три постачальники), 0 помилок; після цього
 * зовнішніх посилань у каталозі не лишилось: 267 із 267 у нашому сховищі.
 */
