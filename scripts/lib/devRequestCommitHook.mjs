...
/**
 * Рядок трейлера. Регістр не важливий, пробіли навколо двокрапки — теж.
 *
 * А ВІДСТУП — ВАЖЛИВИЙ, і це не причіпка. Спершу тут стояло `^[ \t]*`, і перший
 * же коміт із цим правилом сам у нього й провалився: у тілі був ПРИКЛАД формату,
 * записаний блоком коду з відступом у чотири пробіли, — і хук прочитав приклад
 * як намір, закривши рівно ту картку, яку годиною раніше довелось повертати
 * SQL-ом по проду. Приклад у тексті виглядає як трейлер, бо він і є трейлер, —
 * єдине, чим вони відрізняються, це відступ.
 *
 * Тому трейлер починається з початку рядка, як і в самого git
 * (`git interpret-trailers` теж не бачить трейлера з відступом). А в прикладі
 * пиши `REQ-N` замість справжнього номера: без цифр згадка не збігається
 * взагалі, тож приклад лишається прикладом.
 */
