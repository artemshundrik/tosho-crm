...
describe("колір словом із назви сторінки", () => {
  ...
  /**
   * СТАТЬ ЛИШАЄТЬСЯ В ПІДПИСІ, і це не багатослівність, а розрізнення. У
   * моделей RAY чоловіча й жіноча версії мають ТОЙ САМИЙ колір і ТУ САМУ ціну
   * (`U0104M-Cream` і `U0104W-CREAM`), тож без статі 14 кольорів дають 28
   * однакових чипів попарно — і вибір робиться навмання.
   */
  it("стать лишається, коли вона єдине, що розрізняє варіанти", () => {
    const hoodie = "Худі утеплене RAY BASIC з начесом";
    expect(raymarketColorWord("Худі чоловіче чорне утеплене RAY BASIC U0401", hoodie)).toBe("чоловіче чорне");
    expect(raymarketColorWord("Худі жіноче бежеве утеплене RAY BASIC U0401", hoodie)).toBe("жіноче бежеве");
  });

  it("а коли стать є в назві моделі — скорочується сама", () => {
    // Обидві назви кажуть «чоловіча», тож у різницю слово не потрапляє.
    expect(raymarketColorWord("Футболка чоловіча червона STEDMАN CLASSIC-T", model)).toBe("червона");
  });
  ...
  it("латинські хвости назви моделі — теж не колір", () => {
    ...
    // Дитячі: «для хлопчика»/«для дівчинки» теж лишаються — це різні товари.
    expect(
      raymarketColorWord("Футболка дитяча для хлопчика біла STEDMAN CLASSIC-T KIDS", "Футболка дитяча STEDMAN CLASSIC-T KIDS")
    ).toBe("для хлопчика біла");
