...
/**
 * Чи схоже це на токен бота: `<числовий id>:<секрет>`.
 *
 * ЧОМУ ПЕРЕВІРЯЄМО ФОРМУ, А НЕ ПРОСТО «РЯДОК Є». Токен приходить із
 * `netlify env:get`, а той віддає не лише значення: у dev-контексті — фразу
 * «No value set in the dev context…», а для секретної змінної — маску
 * `****************_Acw`. Обидва рядки непорожні. 13.09.2026 сторож зібрав із
 * такого рядка адресу `api.telegram.org/bot…/sendMessage`, отримав 404 і
 * написав «Telegram відповів 404» — тобто звинуватив Telegram у тому, що
 * токена в нього не було зроду. Сторож, який помиляється в діагнозі власної
 * німоти, гірший за німого: його алерт шукають не там.
 */
export function looksLikeTelegramToken(value) {
  return /^\d{5,}:[A-Za-z0-9_-]{30,}$/.test(String(value ?? "").trim());
}
