...
describe("приклад формату в тілі коміта", () => {
  it("трейлер із відступом зупиняє коміт як проза", () => {
    // Реальний випадок: перший же коміт із новим правилом пояснював формат
    // блоком коду — і хук закрив по прикладу справжню картку.
    const message = [
      "Гак коміта більше не закриває картку, згадану мимохідь",
      "",
      "Тепер намір заявляється рядком:",
      "",
      "    Закриває: REQ-17",
    ].join("\n");
    const problems = checkMessage(message);
    expect(problems).toHaveLength(1);
    expect(problems[0]).toMatchObject({ kind: "prose", number: 17, line: 5 });
  });

  it("той самий приклад із `REQ-N` проходить", () => {
    expect(checkMessage("fix: щось\n\nФормат такий:\n\n    Закриває: REQ-N")).toEqual([]);
  });

  it("підказка називає обидва виходи — і слова, і `REQ-N`", () => {
    const text = formatProblems(checkMessage("fix: щось\n\n    Закриває: REQ-17"));
    expect(text).toContain("REQ-N");
    expect(text).toContain("з відступом це приклад");
  });
});
