...
export function raymarketColorWord(siteName, modelName) {
  ...
  const extra = siteName
    ...
    .filter((word) => {
      ...
      // ⚠️ СТАТЬ ВІДСІЮВАТИ НЕ МОЖНА, хоч і кортить: підпис «чоловіче чорне»
      // виглядає багатослівним. Спершу я її прибирав — і 14 кольорів
      // U0104 перетворились на 28 чипів по два однакових («бежева, бежева»),
      // бо чоловіча й жіноча моделі різняться ЛИШЕ статтю: `U0104M-Cream` і
      // `U0104W-CREAM` мають той самий колір і ту саму ціну. Підпис, який не
      // розрізняє варіанти, — це саме той випадок, коли менеджер тицяє
      // навмання, а в замовлення їде артикул не того крою (та сама причина, з
      // якої в Trele в підпис додали розмір).
      //
      // Там, де стать Є в назві моделі («Футболка чоловіча STEDMAN CLASSIC-T»),
      // вона скорочується сама: слово є в обох назвах, у різницю не потрапляє,
      // і чип лишається коротким — «темно-сіра».
