 ...
 *
 * Без відступу, як і в самому хуці: рядок з відступом — це приклад у тексті, а
 * не намір. Він і так не пройде повз гейт, але не як «порожній трейлер», а як
 * звичайна згадка в прозі — з правильною підказкою написати `REQ-N`.
 */
...
export function formatProblems(problems) {
  ...
  out.push("  • це приклад формату в тексті → став `REQ-N` замість справжнього номера");
  out.push("");
  out.push("Трейлер читається лише з початку рядка: з відступом це приклад, а не намір.");
  out.push("");
