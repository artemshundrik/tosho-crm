...
describe("hasOpenChecklist", () => {
   ...
   * що чекає CEO, зроблено не буде ніколи, поки CEO не відповість, — і саме
...
describe("гейт на деплої", () => {
   ...
   * із питанням, що чекало CEO тринадцятий день.
   */
...
describe("pickCardsToCatchUp", () => {
   ...
   * переважно БЕЗ коду — відповів CEO, проклацали, домовились. Нового коміта
