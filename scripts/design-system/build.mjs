...
// Індекс карток для панелі Claude Design.
//
// НАВІЩО ЙОГО ПИСАТИ САМИМ. Панель читає `_ds_manifest.json`, а не файли з
// маркером `@dsCard`. Програма генерує його своєю самоперевіркою — але лише
// раз: після першого заливання. Заміряно 22.08.2026 — у маніфесті лишалось
// шість карток із девʼяти, і три новіші («Глибина», «Спливні шари», «Картка
// канбану») просто не показувались, хоч файли лежали на місці й були цілі.
// `register_assets` тут не рятує: він пише в інший, легасі-реєстр — після
// нього маніфест лишився байт у байт тим самим.
//
// Тому список карток збирається тут, із того самого CARDS, що й самі картки:
// поки джерело одне, розійтися нема чому.
//
// Але ЦІЛИЙ маніфест звідси писати НЕ МОЖНА: крім `cards`, у ньому є
// `namespace` і поля, які заповнює самоперевірка програми (`components`,
// `tokens`, `themes`, `fonts`). Затерти їх — зламати те, чого ми не писали.
// Тому тут лише фрагмент, а при заливанні він ВКЛАДАЄТЬСЯ в наявний маніфест:
//   get_file _ds_manifest.json → замінити поле cards → write_files
writeFileSync(
  path.join(OUT, "_ds_manifest.cards.json"),
  JSON.stringify(CARDS.map(([rel, , group]) => ({ path: `${rel}.html`, group })), null, 0)
);

console.log(`\nГотово: ${OUT}`);
console.log("  _ds_manifest.cards.json — список карток для панелі.");
console.log("  Заливаючи: прочитати _ds_manifest.json проєкту, підмінити в ньому");
console.log("  поле cards цим списком і записати назад — решту полів не чіпати.");
