...
begin
  ...
  -- Той, хто зникає, записується ВЛАСНОЮ назвою («Шовкотрафарет», «УФ дрк»), а
  -- той, хто лишається, — канонічною з довідника. Інакше в журналі стояло б
  -- «Шовкодрук → Шовкодрук», і сліду того, що саме злили, не лишилось би.
  select m.team_id, m.kind_id, m.name
...
-- має називатись. Три групи рядків:
--
-- 1. Вибір одного написання з кількох однакових («УФ - друк» → «УФ-друк»).
-- 2. Одруківки (уфдрк, шоврон, шиврон, термотранфер): зводяться до сусіда з
--    іншим ключем, тобто зникають як окремий метод.
-- 3. Синоніми — рішення Артема від 2026-08-17, це питання виробництва, а не
--    написання: ДТФ = DTF, УФ = УФ-друк, Шовкодрук = Шовкотрафарет =
--    Трафаретний друк, Тампо = Тамподрук, ДТФ-друк = ДТФ, УФ-ДТФ друк = УФ-ДТФ.
--    Канонічне написання в кожній парі — те, що частіше в самій базі
--    (ДТФ 15 видів проти DTF 8, УФ-друк 22 проти УФ 8, Шовкодрук 13 проти
--    Шовкотрафарет 9). Щоб перевернути вибір, досить поміняти рядок нижче.
--
-- Свідомо НЕ зведені (різні процеси, а не написання): Гравіювання vs Лазерне
-- гравіювання, Смоляна наліпка vs Смоляна шильда vs Шильда смоляна,
-- Термотрансфер (DTF) vs ДТФ.
...
--
-- Два тригери на quote_items треба зняти на час злиття, бо вони описують
-- правила ЛЮДСЬКОГО редагування, а не переливання даних:
...
--     Перерахунок тут і не потрібен: заміна method_id не чіпає ні кількостей,
--     ні цін, тож сума прорахунку змінитись не може.
--   * trg_quote_lock_quote_items → «Quote is locked by another user», якщо в
--     мить міграції хтось тримає прорахунок відкритим.
-- Тригер аудиту лишаємо ввімкненим: правка має лишити слід.
-- Усе в одній транзакції, тож при будь-якій помилці тригери повертаються самі.
