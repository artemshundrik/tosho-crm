 ...
 *
 * ПІДНЯТО 06.09.2026 для QuoteDetailsPage (+16) — питання перед зміною ціни,
 * яку клієнт уже погодив (REQ-178#p10). Спершу винесли все, що виноситься:
 * правило живе чистою функцією в `quote-details/approvedRunPriceGuard.ts` (з
 * тестами), стан вікна й саме вікно — у `useApprovedRunPriceGuard.tsx`. У
 * сторінці лишилось те, чого не винести: імпорт, виклик хука, монтування вікна
 ...
 * дереві позиції.
 *
 * ПІДНЯТО 11.09.2026 для QuoteDetailsPage (+14) — роль позиції «варіант»
 * (REQ-267#p2). Винесено все, що виноситься: правило й арифметика меж живуть у
 * `lib/quoteItemVariants.ts` (з тестами), запис прапорця разом із читанням
 * metadata перед записом — у `quote-details/QuoteItemVariantToggle.tsx`. У
 * сторінці лишилось протягування: два імпорти, шість рядків монтування пункту
 * меню й один рядок у паспорті товару, решта +5 — пояснення, навіщо вони там.
 *
 * ПІДНЯТО 11.09.2026 для QuoteDetailsPage (+6) — рядок «з них варіанти» поруч
 * із підсумком картки (доповнення до REQ-267#p2). Межі рахує
 * `lib/quoteItemVariants.ts`, вибір позицій і запам'ятовування —
 * `quote-details/useQuoteVariantRange.ts`, показ — `QuotePriceSummary`. У
 * сторінці лишились два рядки протягування (імпорт і виклик хука, проп у
 * підсумку) і три рядки пояснення, навіщо поруч із сумою стоїть друге число.
 *
 * ОПУЩЕНО 16.09.2026 для обох сторінок (QuoteDetailsPage −22, QuotesPage −46):
 * роль позиції «варіант того самого виробу» прибрана цілком — нею не
 * скористались жодного разу за весь час (0 позначених позицій із 340 на
 * проді). Разом із нею пішли обидва підняття вище: і +14 на сам пункт меню, і
 * +6 на рядок «з них варіанти» поруч із підсумком. Стеля опущена до нового
 * розміру: підняття було позичкою під фічу, і коли фічі немає, позичка не має
 * лишатись висіти.
 *
 * ОПУЩЕНО 11.09.2026 для QuotesPage (−21): попередження про варіант без ціни й
 * підсумок прев'ю КП поїхали в
 * `features/quotes/commercial-document/CommercialPreviewSummary.tsx` — вони
 * читають лише готовий `doc`, тож у сторінці тримати їх не було за що.
 *
 * ОПУЩЕНО 11.09.2026 для QuotesPage (−349): увесь документ КП — типи, формати
 * чисел, HTML для друку/PDF і TSV — поїхав у
 * `features/quotes/commercial-document/document.ts`. Це були чисті функції
 * всередині компонента, тобто 380 рядків, які не мали як бути покритими
 * тестами; тепер вони покриті.
 *
 ...
 * ПІДНЯТО 21.09.2026 для QuotesPage (+5) — дошка прорахунків розтягує колонки
 * на всю ширину полотна (REQ-300). Виносити тут нічого, бо все, що виносилось,
 * уже винесене: сама формула ширини живе в `lib/kanbanColumnWidth.ts` і
 * рахується з реєстру колонок, звідти ж її беруть реєстр поверхонь і дошка
 * беклогу. У сторінці лишилось протягування — імпорт, `style` на колонці й
 * той самий проп каркасу, — і кожен із трьох рядків мусить стояти саме там,
 * де він є: інакше каркас і дошка малюють колонки різної ширини.
 *
 */

const CEILINGS = {
  "src/pages/DesignTaskPage.tsx": 12874,
  // +1 (11.09.2026): рядок імпорту замість вкопаного переліку пресетів — той самий
  // union був продубльований у пʼятьох файлах (REQ-36#f2).
  // +6 (11.09.2026): рядок «з них варіанти» поруч із підсумком картки — межі
  // й вибір позицій винесені в модулі, у сторінці лишилось протягування.
  // +1 (12.09.2026): проп `metadata` у вікні заміни товару (REQ-36#p41). Саме
  // правило «що злітає при зміні виду» винесене в `modelSwapPatch.ts` і вкрите
