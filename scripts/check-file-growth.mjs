 ...
 * рядок вище: спільний перемикач `OffBoardViewSwitch`, обидва списки
 * (`CancelledQuotesList`, `CancelledDesignTasksList`) і дія повернення
 * (`restoreQuote.ts`) — чотири нові модулі, з них два спільні. Те, що лишилось
 * у сторінках, окремо жити не може: це стан вигляду, який читається з фільтра
 * статусу тієї ж сторінки, і гілка розмітки, яка вибирає між дошкою та
 * списком. Винести її означало б винести саму сторінку.
 *
...
const CEILINGS = {
  ...
  "src/pages/QuotesPage.tsx": 8332,
  "src/pages/DesignPage.tsx": 5990,
