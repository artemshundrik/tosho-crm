...
const ACCOUNT = {
  ...
  jobRole: "seo",
  /**
   * ADMIN, А НЕ MEMBER, І ЦЕ ПЕРЕВІРЕНО НА ЖИВОМУ (11.09.2026).
   *
   * Знижували до `member` і повернули: сторінки відкриває посада, але три
   * набори даних висять саме на ролі — ставки й закриття місяця у «Виплатах»
   * (`employee_pay_*`), журнал помилок (`runtime_errors`) та історія
   * працевлаштування. Під `member` RLS віддає їх ПОРОЖНІМИ, без жодної
   * помилки, — тобто знімок виглядає справним, а половини даних на ньому
   * немає. Це найгірший з можливих станів для набору, який саме й існує заради
   * знімків.
   *
   * Чого знижка НЕ дає, хоч на це й розраховували: бота вона не прибирає з
   * отримувачів фінансових сповіщень — `isFinanceRole` пускає за посадою СЕО
   * незалежно від ролі.
   */
  workspaceRole: "admin",
