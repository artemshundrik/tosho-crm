...
/**
 * Політики, які вирішують доступ по членству, але НЕ доходять до гейта
 * заблокованих (REQ-257).
 *
 * НАВІЩО ОКРЕМА ПЕРЕВІРКА. Канонічний `is_team_member(team_id)` тримає в собі
 * дві умови — членство і `not is_user_blocked(auth.uid())`. Обидві потрібні, але
 * друга непомітна: вона всередині функції, і той, хто переписує політику «щоб
 * швидше», бачить лише першу. Саме так виглядає найдешевша помилка в цьому
 * місці — розкласти хелпер на частини й забути половину. Політика лишиться
 * робочою, тести зеленими, а звільнений співробітник знову бачитиме дані.
 *
 * ЩО САМЕ ЛОВИМО: політику, яка сама дістає членство (`get_my_team_ids`, сирий
 * `team_members`/`tosho.memberships`, двоаргументні `is_workspace_admin/owner`)
 * і при цьому ніде не згадує `is_user_blocked`. Політика, що йде через
 * одноаргументний хелпер, чиста — гейт у ньому.
 *
 * ЧОГО НЕ ЛОВИТЬ. Ми дивимось на ТЕКСТ політики, а не розкручуємо виклики
 * рекурсивно. Новий хелпер, який усередині сам загубить гейт, звідси не видно —
 * тому й правило в DB_MAP каже звіряти вирази на живих членах, а не покладатись
 * на цю перевірку саму по собі.
 *
 * ДВА РЯДКИ НИЖЧЕ — ПЕРЕВІРЕНИЙ ВИНЯТОК, А НЕ ДІРКА. Обидві політики кличуть
 * ДВОАРГУМЕНТНІ `is_workspace_admin/owner(workspace_id, auth.uid())`, у яких
 * гейта справді немає, — тому текстова перевірка їх і бачить. Але гейт тут дає
 * RLS: двоаргументні версії, на відміну від одноаргументних, звичайні STABLE без
 * security definer і без `row_security=off`, тож читають `tosho.memberships`
 * ЧЕРЕЗ RLS. Політика читання `memberships_select_member` спирається на
 * block-aware `is_workspace_member`, і для заблокованого таблиця порожня —
 * включно з його власним рядком. А UPDATE/DELETE з колонками таблиці у WHERE
 * додатково вимагають SELECT-політик, тож рядок не знаходиться взагалі.
 *
 * ПЕРЕВІРЕНО НА ПРОДІ 09.09.2026 (усе в begin/rollback, з позитивним контролем):
 * заблокований адмін — `DELETE 0`, заблокований власник — `UPDATE 0`, видалення
 * ВЛАСНОГО членства — `DELETE 0`, активний адмін — `DELETE 1`. Той самий
 * двоаргументний хелпер в обхід RLS повертає `true`, тобто тримає саме RLS.
 *
 * ЧОМУ ЛИШАЄМО В СПИСКУ. Захист непрямий: він тримається на тому, що політика
 * читання `memberships` лишиться block-aware. Зміниться вона — і ці дві політики
 * мовчки відкриються, а перевірка про них уже не нагадає. Зміцнення (перевести на
 * ОДНОАРГУМЕНТНІ хелпери, які несуть гейт у собі) описане в картці 258; це прод-RLS
 * на привілейованому записі, тож лише за окремим рішенням і з rls-verifier.
 */
export const POLICIES_WITHOUT_BLOCK_GATE = new Set([
  "tosho.memberships: memberships_delete_admin_or_self",
  "tosho.memberships: memberships_update_owner_role_only",
]);
