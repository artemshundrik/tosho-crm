...
as $fn$
  ...
  hit as (
    ...
    where pat.arr is not null
      and sp.is_active
      -- ЧУЖА ЦІНА ГІРША ЗА ЖОДНУ (Артем, 08.09.2026). Ховаємо рядки, у яких
      -- ціна є, але вона НЕ наша: `price_kind = 'retail'` при заповненій ціні
      -- означає вітрину постачальника, з якої нічого не порахуєш. Порожнє поле
      -- менеджер помітить і піде питати; чуже число він просто візьме.
      --
      -- Сьогодні під це підпадає рівно Беррітекс (2098 рядків) — у нього ще
      -- немає домовленості про знижку. Аванпринт лишається видимим: у нього
      -- ціна порожня (він тут заради артикулів, фото й описів під специфікації),
      -- Тотобі й Бергамо — теж, у них `price_kind = 'wholesale'`.
      --
      -- Правило, а не список імен, навмисно: щойно Беррітексу пропишуть
      -- `priceRule` у реєстрі завантажувача, він повернеться в пошук сам, і
      -- нікому не доведеться згадувати, що його колись сховали тут.
