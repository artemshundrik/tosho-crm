...
- **Перевірка: `npm run check:fast` під час роботи, `npm run check` перед словами «готово до викочування».** `typecheck` + `lint` — це НЕ «чисто»: чотири з дев'ятнадцяти перевірок (станом на 07.09.2026) ратчети й ростуть непомітно. Окремі команди — для точкової діагностики; повна перевірка тепер 11 с.
...
Ти — диригент; агенту — лише те, що окупається. Ціни й заміри — у [rationale](docs/CLAUDE_RULES_RATIONALE.md#витрати-звідки-взялися-цифри).

- **Дрібне — сам.** Знаєш файл і правка на десятки рядків — руками: агент дорожчий за правку.
- **Шукати — `Explore` з `model: "haiku"`**; знаєш де — клади вміст у бриф, удвічі дешевше.
- **Механіка з готовим ТЗ і кодом у брифі — агент `implementer`** (Sonnet). Без `model` агент і так іде на Sonnet; Opus — лише явно, Fable — на прохання.
- **Рецензент — лише де помилка дорога**: база, RLS, автентифікація, спільні примітиви, гроші — там `model: "opus"`. Механічний UI — досить `check:fast`.
- **Роботу агента перевір сам** (дифф + `check:fast`). Двох агентів на одне не став.
...
`QuoteDetailsPage.tsx` (~6850 рядків), `DesignTaskPage.tsx` (~12864), `QuotesPage.tsx` (~6952), `DesignPage.tsx` (~5937) — станом на 24.09.2026. `Read` ріже по 2000 рядків, тож стрибай за офсетами з [docs/LARGE_FILES_MAP.md](docs/LARGE_FILES_MAP.md), а не скануй з першого рядка.
