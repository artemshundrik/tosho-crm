...
- **NEVER `git push` on your own initiative.** Пуш = прод-деплой = ~15 кредитів із ≈40 на місяць. Комить локально скільки треба, перевіряй `npm run check`, закінчуй хід звітом «накопичено N комітів, готові до викочування» + список — і чекай. Пушиш лише на «пушимо»/«викочуй»/«деплой» або на гарячий фікс зламаного проду. Гак це стереже: не обходь його й не повертай `Bash(git push *)` в `allow`.
- **Перевірка: `npm run check:fast` під час роботи, `npm run check` перед словами «готово до викочування».** `typecheck` + `lint` — це НЕ «чисто»: чотири з шістнадцяти перевірок ратчети й ростуть непомітно. Окремі команди лишаються для точкової діагностики.
- **Типи перевіряє TypeScript 7 і тільки повним шляхом** — `node node_modules/tsc7/bin/tsc`. Голий `tsc`/`npx tsc` мовчки стає сімкою через аліас; у нових скриптах пиши повний шлях.
...
`QuoteDetailsPage.tsx` (~9900 рядків), `DesignTaskPage.tsx` (~12864), `QuotesPage.tsx` (~8380), `DesignPage.tsx` (~6010) — станом на 27.08.2026. `Read` ріже по 2000 рядків, тож стрибай за офсетами з [docs/LARGE_FILES_MAP.md](docs/LARGE_FILES_MAP.md), а не скануй з першого рядка.
