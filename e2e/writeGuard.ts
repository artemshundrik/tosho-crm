 ...
 *   • `/storage/v1/object/sign/…` — підпис посилання на ЗАВАНТАЖЕННЯ файлу.
 *
...
export function classifyRequest(
  ...
  if (parsed.pathname.startsWith("/storage/v1/")) {
    /**
     * ПІДПИС ПОСИЛАННЯ — ЦЕ ЧИТАННЯ, хоч і POST. `POST /storage/v1/object/sign/…`
     * не створює й не міняє файл: він видає тимчасове посилання, щоб приватний
     * файл можна було ЗАВАНТАЖИТИ. Саме так у застосунку показується кожна
     * аватарка.
     *
     * Поки це глушилось, перший же прогін під справжнім акаунтом (11.09.2026)
     * дав по два десятки «спроб запису» на кожен сценарій — і всі до одної
     * фальшиві, від аватарок у шапці. Червоне, яке нічого не означає, вчить не
     * читати звіт, тож ціна такої «обережності» вища за користь.
     *
     * `/storage/v1/object/upload/sign/…` — інший шлях і справжній запис: він
     * під цей виняток не підпадає, бо починається з `/upload/`.
     */
    if (parsed.pathname.startsWith("/storage/v1/object/sign/")) return { blocked: false };
