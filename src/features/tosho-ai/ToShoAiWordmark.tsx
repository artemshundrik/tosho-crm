/**
 * Логотип ToSho AI. Живе ОКРЕМО від ToShoAiConsole навмисно: шапка шторки
 * рендерить wordmark одразу, а сама консоль (110+ KB) вантажиться lazy при
 * першому відкритті. Якби wordmark лишався в модулі консолі, його імпорт у
 * AppLayout тягнув би всю консоль у головний чанк — саме так і було.
 */
/**
 * Сам знак ToSho AI — дві зірки. Окремим експортом, бо його носить не лише
 * wordmark: та сама позначка стоїть у полі пошуку в шапці й на рядку «Спитати
 * ToSho AI» в палітрі. Доти там була універсальна іскра з lucide, і позначок
 * у продукту виходило дві.
 */
export function ToShoAiMark({ className }: { className?: string }) {
  return (
    <svg viewBox="0 0 33 33" className={className} fill="none" xmlns="http://www.w3.org/2000/svg" aria-hidden="true">
      <path
        d="M13.9446 6.46076C14.2193 5.34828 15.8008 5.34828 16.0754 6.46076C17.2477 11.2094 20.9555 14.9171 25.7041 16.0893C26.8167 16.3639 26.8167 17.9455 25.7041 18.2201C20.9555 19.3924 17.2477 23.1 16.0754 27.8487C15.8008 28.9611 14.2193 28.9611 13.9446 27.8487C12.7723 23.1 9.06455 19.3924 4.31589 18.2201C3.20337 17.9455 3.20337 16.3639 4.31589 16.0893C9.06455 14.9171 12.7723 11.2094 13.9446 6.46076Z"
        fill="currentColor"
      />
      <path
        d="M25.6579 1.74675C25.7691 1.29646 26.4092 1.29646 26.5204 1.74675C26.9949 3.66882 28.4957 5.16953 30.4177 5.64401C30.868 5.75518 30.868 6.39533 30.4177 6.50649C28.4957 6.98097 26.9949 8.48169 26.5204 10.4038C26.4092 10.854 25.7691 10.854 25.6579 10.4038C25.1834 8.48169 23.6827 6.98097 21.7606 6.50649C21.3103 6.39533 21.3103 5.75518 21.7606 5.64401C23.6827 5.16953 25.1834 3.66882 25.6579 1.74675Z"
        fill="currentColor"
      />
    </svg>
  );
}

export function ToShoAiWordmark() {
  return (
    <div className="inline-flex items-center gap-2.5">
      <div className="flex h-9 w-9 items-center justify-center rounded-full bg-ai-accent/12 text-ai-accent ring-1 ring-ai-accent/18">
        <ToShoAiMark className="h-[18px] w-[18px]" />
      </div>
      <span className="text-[18px] font-semibold tracking-[-0.03em] text-ai-accent md:text-[20px]">ToSho AI</span>
    </div>
  );
}
