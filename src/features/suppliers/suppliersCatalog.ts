...
export const SUPPLIER_DEFINITIONS: readonly SupplierDefinition[] = [
  ...
  },
  {
    ...
    cabinetUrl: "https://toptime.com.ua/login/",
    platform: "власний рушій",
    sells: "Однотонний одяг і головні убори під друк: Stedman, Gildan, Just Hoods, Kariban, SOL'S, Headwear",
    intake: {
      kind: "crawl",
      summary:
        "Обхід 166 сторінок товару під логіном. Фіда немає; перелік адрес беремо зі сторінки «Все», а не з мапи сайту — мапа протухла, 58 адрес із 181 віддають 404.",
      schedule: "daily",
      scheduleNote:
    ...
    price: {
      basis: "account",
      summary:
        "Сайт показує драбину з чотирьох сходинок за кількістю; беремо найглибшу — партнерську, це 0,73–0,74 від рекомендованої. Решта сходинок лежить поруч у рядку, тож дрібний тираж можна порахувати без другого походу на сайт.",
      agreedOn: "2026-09-10",
      vat: "Ціни на сайті з ПДВ — написано під таблицею цін, тому й наші з ПДВ.",
    },
    access: { emailEnv: "TOPTIME_EMAIL", passwordEnv: "TOPTIME_PASSWORD" },
    gives: [
      "артикул моделі",
      "наша ціна",
      "уся драбина цін",
      "колір із кодом",
      "розміри",
      "залишок по кожній парі «колір + розмір»",
      "запас у Європі",
      "фото на кожен колір",
      "бренд",
      "розділ",
      "методи нанесення",
      "матеріал, щільність, покрій, штук у коробці",
    ],
    lacks: ["фід у будь-якому вигляді", "hex кольору", "окрема ціна на розмір (3XL-5XL дорожчі, але скільки — питати менеджера)"],
    quirks: [
      "Мапа сайту протухла: третина адрес товарів у ній — 404. Перелік беремо зі сторінки «Все», вона сама каже, скільки товарів має.",
      "Гість бачить лише першу колонку цін, решта три — посилання «Реєстрація». Тому відпалу сесію видно одразу: рядки лишаються без ціни, а не дешевшають.",
      "Ціни два рядки, «White» і «Color»: білий дешевший. У кожної моделі рівно один колір названий дослівно «White», решта білих відтінків (Absolute White, Off White) свідомо йдуть за дорожчим рядком.",
      "Назви на сайті англійські («Classic Men»), тому в пул вони їдуть з українським словом розділу спереду — інакше на запит «футболка» джерело мовчало б усі 166 товарів.",
      "Бренда на сторінці товару немає — його знає лише фільтр каталогу, і завантажувач питає його шістьма окремими запитами.",
    ],
    inQuoteSearch: true,
  },
  {
  ...
  },
  {
    ...
    slug: "midocean.com",
    name: "Midocean",
    siteUrl: "https://www.midocean.com/central-europe/us/huf/",
    platform: "Intershop (Angular PWA)",
    sells: "Європейська промо-продукція: ручки, пляшки, сумки, записники, різдвяна серія",
    intake: {
      kind: "feed",
      summary:
        "Прайс приходить книжкою Excel від менеджера, а фото не шукаються взагалі: адреса знімка будується просто з артикула. Обходу немає, прогін секундний.",
      schedule: "manual",
      scheduleNote:
        "Руками, коли приїде новий прайс. Ціни midocean міняє раз на квартал-півроку, тож щоденному крону тут нема чого робити — у supplier-feeds це джерело не стоїть.",
    },
    price: {
      basis: "account",
      summary:
        "Ціна в прайсі вже наша, дилерська (тариф Д1). У пул іде в ЄВРО: у файлі є й гривня, але порахована курсом дня вивантаження (53,65 у серпневому прайсі, один на всі 5441 рядки) — через місяць таке число бреше. Гривня з курсом лежать поруч в attrs.",
      agreedOn: "2026-09-16",
      vat: "Ціни в Д1 з ПДВ (Артем, 16.09.2026), тому й наші з ПДВ.",
    },
    gives: ["артикул", "назва українською", "наша ціна в євро", "фото"],
    lacks: [
      "розділ",
      "колір назвою (у артикулі є лише код)",
      "методи нанесення",
      "розміри поля друку",
      "опис",
    ...
    quirks: [
      "Артикул прайса — це SKU midocean у масці: MMO320003X читається як MO3200-03. Розкладаються всі 5441 рядок серпневого файлу.",
      "Фото не обходиться, а будується: cdn1.midocean.com/image/700X700/{sku}.jpg. На 250 перевірених артикулах кадр знайшовся 250 разів, але кожна адреса все одно перепитується — зняті з виробництва позиції лишаються в прайсі без знімка.",
      "Обхід сайту тут не годиться: розділи малюються вже в браузері, а адреса картки товару містить їхній внутрішній id (mo3200-03-zid10248207), якого в прайсі немає. Мапи сайту теж немає — sitemap.xml віддає оболонку застосунку.",
      "У midocean є офіційне Customer API (products, pricelist, stock, printdata, printpricelist) — воно просить заголовок з ключем, який видає їхній менеджер. Дадуть ключ — звідти доїдуть кольори, розділи, нанесення й розміри, і цей прайс лишиться при самих цінах.",
      "Назва в прайсі інколи містить код базового кольору моделі, а не свого: «Косметичка двоколірна MO9815-03» стоїть підписом і в MO9815-05, і в MO9815-10. Правду про колір каже тільки артикул.",
    ],
    inQuoteSearch: true,
  },
  {
    ...
    name: "Raymarket",
    siteUrl: "https://ray-market.com.ua/",
    platform: "OpenCart",
    sells: "Київська фабрика рекламного текстилю: власні моделі RAY і Stedman",
    intake: {
      kind: "crawl",
      summary:
        "Фіда немає (перевірено 16.09.2026). Перелік адрес — мапа сайту, дані — зі сторінок товарів. Ціни сайт не знає взагалі: вони приходять двома PDF-прайсами руками.",
      schedule: "manual",
      scheduleNote:
        "Руками, коли приїде новий прайс. Обхід 666 адрес займає майже 6 хвилин і тягне близько 290 МБ: сторінки в них важкі (по 440 кБ) і віддаються по 4 секунди. Заради даних, що міняються з новим PDF, у денному розкладі supplier-feeds це джерело не стоїть.",
    },
    price: {
      basis: "account",
      summary:
        "Ціна з прайсів «ВЕЛИКИЙ ОПТ» (зима й літо, 11.05.2026) — вона вже наша. Множника від роздробу немає: заміряно 0,45–0,64 на різних групах, тож правилом її не вивести. У пул іде ціна базового розмірного діапазону, уся драбина — в attrs.",
      agreedOn: "2026-05-11",
      vat: "Колонка прайса підписана «Ціна з ПДВ», тому й наші з ПДВ.",
    },
    gives: [
      "артикул на кожен колір",
      "назва",
      "бренд",
      "розділ",
      "опис зі складом і щільністю",
      "фото (усі 854×1278, жодної заглушки)",
      "наявність",
      "наша ціна з прайса",
      "розмірна драбина цін",
    ],
    lacks: [
      "ціни на 124 артикули зі 149, яких немає у двох прайсах",
      "розміри окремими рядками (у прайсі лише діапазони)",
      "методи нанесення",
    ],
    quirks: [
      "Сайт показує РОЗДРІБ, і показує його навіть під логіном — постачальник пише це в самому прайсі. Тому ціна береться з PDF, а роздріб лишається в attrs.sitePrice для звірки.",
      "Білий дешевший за кольоровий, і тільки у Stedman: ST2000 білий 139,50, кольоровий 168. У власних моделях RAY ціна одна на всі кольори.",
      "Великий розмір дорожчий: XS-2XL за одну ціну, 3XL-5XL за іншу. У картці стоїть ціна базового діапазону, решта драбини — поруч.",
      "У мапі сайту 80 мертвих адрес із 666 (12%, усі — чесний 404). Обхід рахує їх зниклими товарами, а не невдачами, інакше зупинявся б на здоровому сайті.",
      "У полі артикула в 37 товарів написано «Під замовлення». Такі рядки лишаються без ціни й у пул не йдуть: вгадати модель із назви можна, але помилка тут коштує чужої ціни в прорахунку.",
      "U0102P і U0104P — це НЕ U0102 і U0104, а модні кольори з власним артикулом, якого в прайсі немає. Усі 95 таких рядків на сайті стоять «немає в наявності» з ціною 0.",
    ],
    inQuoteSearch: true,
  },
];
