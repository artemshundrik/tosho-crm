  ...
  const handleAddSupplierProduct = (product: SupplierPoolProduct) => {
    ...
    */
    ...
    const draft = makeDraft({
      ...
      supplierProductId: product.priceRowId,
      catalog: guess
        ? {
            modelId: null,
            kindId: guess.kindId,
     ...
     * того, хто читає список; ця ловить того, хто його проглянув (Артем,
     * 16.09.2026: «і підказка»).
     *
     * Не питання й не заборона. Позиція лягає в чернетку як завжди, бо дитяча
     * модель — нормальний товар, і вимагати підтвердження на кожну означало б
     * навчити клацати «так» не читаючи. Завдання підказки одне: назвати вголос
     * те, чого менеджер не збирався вибирати, поки прорахунок ще чернетка.
     */
    if (product.isKids) {
      toast.warning("Додано дитячу модель", {
        description: `«${product.name}» — дитячий крій. Якщо мала бути доросла, приберіть позицію й пошукайте ту саму модель без позначки «Дитяча».`,
        duration: 8000,
      });
    }
          ...
          <div className="flex min-w-0 flex-1 flex-col md:min-h-0">
            ...
            <DialogFooter className="shrink-0 gap-2 border-t border-border/60 px-5 py-3 sm:justify-between">
              ...
              */}
              ...
              <span className="min-w-0 flex-1 self-center truncate text-xs text-muted-foreground" title={footerMeta}>
                {footerMeta}
              </span>
    ...
    return (
      <div className="space-y-3">
        <div className="flex items-center gap-3 rounded-xl border border-border/60 px-3 py-2.5">
          {/*
            Жива ілюстрація замість статичної піктограми (REQ-253). Розбір
            моделлю триває до пів хвилини, і весь цей час панель не мала
            жодного руху: три кроки словами перемикаються рідко, а між ними
            екран виглядав завислим. Рух тут нічого не вимірює — він лише
            каже «працюю», а що саме роблю, кажуть кроки нижче.
          */}
          <span className="grid h-10 w-10 shrink-0 place-items-center text-muted-foreground">
            <UploadIllustration className="h-10 w-10" />
          </span>
