...
describe("guessKindFromTitle", () => {
  ...
  /**
   * Категорія постачальника як джерело виду (REQ-250#p3). Товар із пулу несе
   * власну класифікацію постачальника — «Футболки», «Поло», «Светри», — і вона
   * надійніша за здогад із назви, бо це не рядок, а те, як постачальник САМ
   * розклав свій асортимент. Окремої функції під це немає навмисно: порівняння
   * по основі слова тут уже є, тож множина «Футболки» знаходить вид «Футболка».
   */
  it("категорія постачальника у множині знаходить наш вид", () => {
    // Одне слово без решти назви — саме так категорія й приходить із фіда.
    expect(guessKindFromTitle(kinds, "Кепки")?.kindName).toBe("Кепка");
    expect(guessKindFromTitle(kinds, "Записні книжки")?.kindName).toBe("Записна книжка");
  });

  it("категорії, якої в нас немає, вид не вигадується", () => {
    // «Розпродаж» і «Sport Line» — це полиці магазину, а не види товару.
    expect(guessKindFromTitle(kinds, "Розпродаж")).toBeNull();
    expect(guessKindFromTitle(kinds, "Sport Line")).toBeNull();
  });
