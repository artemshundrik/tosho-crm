 ...
 *
 * Собівартість нулями, КРІМ вартості товару, коли джерело її назвало: товар із
 * пулу приходить із нашою ціною, і змушувати менеджера переписувати число, яке
 * він щойно бачив у підказці, безглуздо (Артем, 08.09.2026). Решта трьох полів
 * (нанесення, логістика, бажаний дохід) і далі нулі — їх не знає ніхто, крім
 * людини. Ставки — не собівартість, а налаштування прорахунку, тож вони
 * приходять як у будь-якому новому тиражі.
 *
 * `unit_price_model_vat` для ціни з фіда — «з ПДВ» (рішення Артема 08.09.2026).
 * Раніше вона лишалась порожньою, бо джерела кажуть різне: у Тотобі ціни з
 * ПДВ, у Бергамо це ніде не записано, а вигадувати факт про гроші не можна.
 * Артем цей факт назвав: ціни постачальників ідемо вважати з ПДВ, і гейт про
 * ПДВ на цих тиражах більше не спрацьовує. Там, де ціни немає, позначка й далі
 * порожня — вона описує суму, якої нема.
 *
...
export function buildImportRunPayloads(input: {
  ...
  return input.draft.runs.map((run) => ({
    ...
    unit_price_model: input.draft.unitCost ?? 0,
    unit_price_model_vat: input.draft.unitCost ? "incl" : null,
    unit_price_print: 0,
    logistics_cost: 0,
