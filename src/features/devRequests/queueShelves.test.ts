...
describe("вибране на сьогодні", () => {
  ...
  /*
   * ВИЧЕРПАНИЙ НАКОПИЧУВАЧ — це його «зроблено».
   *
   * Дірка, що відкрилась 27.08.2026, коли REQ-180 став ПЕРШИМ накопичувачем,
   * у якого закрились геть усі пункти. Статус у нього завжди `queued`, тож
   * чистка полиці, яка дивиться лише на статус, не прибирала його НІКОЛИ:
   * «Сьогодні» рахувало його як одну справу на день, хоч робити там уже нічого,
   * а рядок на тій полиці навіть не показує лічильника — порожній накопичувач
   * не відрізнити від повного.
   */
  it("накопичувач без відкритих пунктів на сьогодні не береться", () => {
    const done = (id: string): ChecklistItem => ({ ...waiting(), id, state: "done" });
    const empty = make("1", "queued", [done("p1")], "Дрібниці: картка прорахунку");
    expect(canTakeToday(empty)).toBe(false);
    expect(pruneToday(["1"], [empty])).toEqual([]);
  });

  it("накопичувач із бодай одним відкритим пунктом брати МОЖНА", () => {
    const mixed = make(
      "1",
      "queued",
      [{ ...waiting(), id: "p1", state: "done" }, { ...waiting(), id: "p2", state: "todo" }],
      "Дрібниці: мова інтерфейсу"
    );
    expect(canTakeToday(mixed)).toBe(true);
  });

  it("накопичувач без жодного пункта теж не береться — розгрібати нічого", () => {
    expect(canTakeToday(make("1", "queued", [], "Дрібниці: гроші замовлення"))).toBe(false);
  });

  it("правило стосується САМЕ накопичувачів — звичайну картку закритий чекліст не блокує", () => {
    // У великої задачі закритий чекліст означає «лишилось викотити», і брати її
    // на сьогодні цілком осмислено — там ще є що доробити руками.
    const usual = make("1", "queued", [{ ...waiting(), id: "p1", state: "done" }], "Звичайна картка");
    expect(canTakeToday(usual)).toBe(true);
  });
});
...
describe("накопичувачі дрібниць", () => {
  ...
  it("на сьогодні його взяти МОЖНА, поки є що розгрібати", () => {
    ...
    // але лишається умова, щоб у списку був бодай один відкритий пункт.
    const full = make("1", "queued", [{ ...waiting(), id: "p1", state: "todo" }], "Дрібниці: мова інтерфейсу");
    expect(canTakeToday(full)).toBe(true);
  });
