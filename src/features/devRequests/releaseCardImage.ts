...
export type ReleaseCardInput = {
  ...
  /**
   * Мітки під назвою: зона → тема → напрямок. Порожні не малюються.
   *
   * ТУТ ЇХ ТРИ, А НА ДОШЦІ ЛИШЕ ТЕМА — і це навмисна різниця, а не розбіжність.
   * Дошку сканують очима по п'ятдесят карток, тож класифікація там була шумом і
   * пішла у фільтри (див. buildCardMeta, 26.08.2026). Картинку ж читає одна
   * людина в чаті, у неї немає ні фільтрів, ні дошки поруч — і «Прорахунки» під
   * назвою для неї єдиний спосіб зрозуміти, де саме шукати зміну.
   *
   * Тому поля тут ОКРЕМІ, а не з buildCardMeta: спільне джерело зв'язало б два
   * різні питання й наступна правка дошки мовчки зіпсувала б картку для чату.
   */
