...
/**
 * Чи картка стоїть не через нас: у її чеклісті є пункт у стані «Чекає».
 *
 * Потрібно рівно для порядку в колонці — див. sortColumn. Окрема функція, бо
 * питання «заблоковано?» задають і оку (жовтий підпис «чекає СЕО · 27 дн» у
 * ChecklistBar), і сортуванню, і одна відповідь на двох краща за дві копії
 * умови, які розійдуться.
 */
export function isBlockedOnPerson(request: DevRequest): boolean {
  return checklistProgress(request.checklist).waiting > 0;
}

/**
 * Порядок карток у колонці.
 *
 * ПЕРШИЙ КЛЮЧ — «чи стоїть воно через людину». Заблоковані опускаються вниз
 * колонки, живі лишаються вгорі. Причина заміряна 26.08.2026: у «В роботі»
 * висіли чотири картки, і три з них насправді чекали відповіді — REQ-15 на СЕО
 * від 30 липня. Колонка з назвою «В роботі», де ніхто нічого не робить, вчить
 * не вірити дошці.
 *
 * ЧОМУ НЕ ОКРЕМА КОЛОНКА «На паузі». Це була перша думка, і вона хибна: п'ята
 * колонка забирає ще шматок ширини й додає ще одне місце, куди дивитись, — на
 * дошці, яка й так перевантажена. Порядок усередині колонки дає те саме
 * розділення безкоштовно: угорі те, що можна брати, унизу те, що стоїть не
 * через тебе, і підпис поруч каже, кого саме чекаємо.
 *
 * ДАЛІ — за релевантною датою (новіші вгорі), а вже потім за номером. Номер як
 * останній ключ — щоб порядок не стрибав, коли кілька карток викотили одним
 * деплоєм і час у них до секунди однаковий.
 */
export function sortColumn(items: DevRequest[], status: RequestStatus): DevRequest[] {
  return [...items].sort((a, b) => {
    const aBlocked = isBlockedOnPerson(a);
    const bBlocked = isBlockedOnPerson(b);
