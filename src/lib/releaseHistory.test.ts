...
describe("дні роботи", () => {
  ...
  it("один пуш за три дні роботи — це три дні", () => {
    // Інакше «змін за день» ділиться на 1 замість 3 і завищує темп утричі.
    const days = workingDays([
      release("2026-08-19T18:00:00Z", [
        change({ sha: "a", at: "2026-08-17T10:00:00+03:00" }),
        change({ sha: "b", at: "2026-08-18T10:00:00+03:00" }),
        change({ sha: "c", at: "2026-08-19T10:00:00+03:00" }),
      ]),
    ]);
    expect(days).toBe(3);
  });
...
describe("групування по днях", () => {
  ...
  it("пуш за три дні роботи розкладається по днях комітів", () => {
    // Справжній випадок 19.08.2026: не пушив три дні, потім віддав 22 коміти
    // одним пушем — і теплокарта показала три порожні дні, хоч у них є години.
    const days = groupByDay([
      release("2026-08-19T18:07:00Z", [
        change({ sha: "a", at: "2026-08-16T11:02:00+03:00" }),
        change({ sha: "b", at: "2026-08-17T22:46:00+03:00" }),
        change({ sha: "c", at: "2026-08-19T20:52:00+03:00" }),
        change({ sha: "d", at: "2026-08-19T21:03:00+03:00" }),
      ]),
    ]);
    expect(days.map((day) => day.day)).toEqual(["2026-08-19", "2026-08-17", "2026-08-16"]);
    expect(days.map((day) => day.changes.length)).toEqual([2, 1, 1]);
  });

  it("день беремо в часі коміта, а не в UTC", () => {
    // Коміт о 00:40 за Києвом — це вже наступний день, і в теплокарті він має
    // стояти саме там, інакше нічна робота падає у вчора.
    const days = groupByDay([
      release("2026-08-19T18:00:00Z", [change({ sha: "a", at: "2026-08-16T00:40:00+03:00" })]),
    ]);
    expect(days[0].day).toBe("2026-08-16");
  });
});
...
describe("групування за місяцями", () => {
  ...
  it("пуш на межі місяців розкладає коміти по своїх місяцях", () => {
    // Пуш 1 вересня із серпневими комітами не має віддавати серпневу роботу
    // вересню: інакше псуються обидва місяці одразу.
    const groups = groupByMonth([
      release("2026-09-01T09:00:00+03:00", [
        change({ sha: "a", at: "2026-08-30T18:00:00+03:00" }),
        change({ sha: "b", at: "2026-08-31T20:00:00+03:00" }),
        change({ sha: "c", at: "2026-09-01T08:00:00+03:00" }),
      ]),
    ]);
    expect(groups.map((group) => group.key)).toEqual(["2026-09", "2026-08"]);
    const changesIn = (key: string) =>
      groups
        .find((group) => group.key === key)!
        .releases.reduce((sum, rel) => sum + rel.changes.length, 0);
    expect(changesIn("2026-08")).toBe(2);
    expect(changesIn("2026-09")).toBe(1);
  });
