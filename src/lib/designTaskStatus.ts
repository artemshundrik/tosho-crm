...
/**
 * Що робити з переходом у «Правки»: пустити, спитати про чернетку чи вимагати текст.
 *
 * ЧОМУ ЦЕ ЧИСТА ФУНКЦІЯ, А НЕ ЛАНЦЮЖОК `if` У СТОРІНЦІ. Розвилка тут не одна:
 * свіжа правка є / немає, чернетка набрана / порожня. Це чотири випадки, і в
 * картці задачі вони губились би серед решти тринадцяти тисяч рядків, де їх
 * ніхто не перевірить. Окремою функцією вони стають переліком, який видно
 * цілком і який тримають тести.
 *
 * `ask_send_draft` навмисно сильніший за `require_text`: якщо текст уже
 * набраний, питати «опишіть правку» було б знущанням — вона описана, її просто
 * не надіслали.
 */
export type ChangesGateOutcome =
  /** Свіжа правка вже є — перехід дозволений. */
  | "allow"
  /** Правки немає, але є набрана чернетка — пропонуємо надіслати саме її. */
  | "ask_send_draft"
  /** Немає ні правки, ні тексту — просимо описати. */
  | "require_text";

export const resolveChangesGate = (
  input: ChangesGateInput & { hasUnsentDraft: boolean }
): ChangesGateOutcome => {
  if (hasFreshChangeRequest(input)) return "allow";
  return input.hasUnsentDraft ? "ask_send_draft" : "require_text";
};
