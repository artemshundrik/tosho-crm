 ...
 *
 * ЧИТАЧІ — ОБИДВА, І ЦЕ НЕ ФОРМАЛЬНІСТЬ:
 *   1. картка задачі — `src/pages/DesignTaskPage.tsx` (`fetchDesignTaskQuoteItem`);
 *   2. дошка «Дизайн» — `src/pages/DesignPage.tsx` (`pickTaskQuoteItem`).
 *
 * Першого полагодили 27.08, другого забули — і на дошці всі задачі одного
 * прорахунку далі показували перший товар, уже після того, як фікс викотили.
 * Правило у власному модулі захищає тільки тих, хто його кличе. Додаєш третього
 * читача — клич звідси й допиши себе сюди; сирий запит до `quote_items` у файлі
 * про дизайн-задачі зупиняє пуш (`scripts/check-rule-readers.mjs`).
 */
