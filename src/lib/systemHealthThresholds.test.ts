...
describe("classifyCronJob", () => {
  ...
  it("колір не суперечить тексту: 1.6 год показується як «2 год» і рахується як 2", () => {
    // 20.08.2026 о 18:20 два рядки одного листа казали «останній 2 год тому»,
    // але один був червоний, а другий жовтий — різниця в 24 хвилини, яких у
    // листі не видно.
    expect(
      classifyCronJob({ hoursSinceLastRun: 0, failures: 51, hoursSinceLastFailure: 1.6 })
    ).toBe("warning");
    // А 1.4 округляється до 1 — і лишається червоним, як і написано в тексті.
    expect(
      classifyCronJob({ hoursSinceLastRun: 0, failures: 51, hoursSinceLastFailure: 1.4 })
    ).toBe("danger");
  });
