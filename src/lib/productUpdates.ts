...
export function planUpdateModal({ unread, now, shownThisSession }: ModalPlanInput): ModalPlan {
  ...
  /**
   * Модалку відкриває лише щось справді помітне. Пачка дрібних покращень —
   * а їх буває по десятку за день — живе в стрічці й бейджі, і не більше:
   * саме так записано в дизайн-доку («косметика → тільки стрічка»).
   *
   * Без цієї умови день дрібних правок вилився б у модалку для всієї
   * команди, тобто рівно в ту поведінку, через яку промо зібрало 53 покази
   * й 2 кліки.
   */
  if (majors.length === 0) return { kind: "none" };
