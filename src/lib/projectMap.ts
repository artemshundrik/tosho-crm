 ...
 *
 * Розбір запиту (netlify/functions/_lib/devRequestDraft.ts) бачить лише
 * надісланий текст: він охайно структурує слова, але сам по собі не знає, з
 * чого складається застосунок — тож не міг би ні поставити напрямок, ні
 * сказати «це вже працює, ось тут». Увесь код у запит не покласти, а карта —
 * можна.
 *
