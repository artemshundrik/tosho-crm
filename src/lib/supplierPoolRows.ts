...
const SUPPLIER_NAMES: Record<string, string> = {
  ...
  // Без запису вийшло б «Ray-market»: дефіс усередині домену відкидання зони не
  // прибирає. Самі вони пишуться «RAY MARKET», але в цьому рядку всі джерела
  // одним словом з великої літери — див. коментар вище.
  "ray-market.com.ua": "Raymarket",
};
...
function mergeCluster(cluster: SupplierPoolDraft[]): SupplierPoolProduct {
  ...
  for (const draft of byContent) {
    ...
    for (const variant of draft.variants) {
       ...
       *
       * А КОЛИ ЦІНА Є В ОБОХ — БЕРЕМО ДЕШЕВШУ (Артем, 10.09.2026). Досі
       * правило звучало «заповни порожнє», тобто перша ціна лишалась назавжди,
       * і друга просто не показувалась. Поки спільні артикули були тільки між
       * оптовиком і нашим магазином, це нічого не значило: в Аванпринта ціни
       * немає взагалі. Із заливом Trele з'явився перший випадок, коли ціну
       * мають ОБИДВА: вісім артикулів Fruit of the Loom є і в Trele, і в
       * Е-Сувеніра, і Е-Сувенір дешевший на 40% (97,17 проти 161,70). Тобто
       * менеджер рахував би за дорожчою, не знаючи, що поруч є дешевша.
       *
       * Посилання при цьому НЕ переїжджає: картка й так показує всі сайти, де
       * ця річ є (`sources`), а адреса варіанта веде туди, звідки взято його
       * назву й фото.
       */
      if (variant.price !== null && (twin.price === null || variant.price < twin.price)) {
        twin.price = variant.price;
        // Разом із ціною переїжджає й адреса її ДЖЕРЕЛА В ПУЛІ: далі база
        // читатиме число саме звідти, а не з рядка, який його не має.
  ...
  //
  // Тут саме ПЕРШИЙ, у кого ціна є, а не той, чия ціна виявилась дешевшою:
  // рядки з роздрібною ціною `search_supplier_pool` не віддає взагалі, тож
  // серед карток із ціною підпис однаковий, і вибирати між ними нема чого.
...
/**
 * ТРИ СИМВОЛИ, А НЕ ДВА, і це не смак, а побудова триграм. Покажчики пулу —
 * `gin_trgm_ops`, тобто шукають за трійками літер. У шаблоні з двох символів
 * повної трійки немає ЖОДНОЇ, тож GIN не відсіює нічого, чесно віддає всю
 * таблицю, і запит вироджується в повний скан: 5,2 с на «фу» проти 70 мс на
 * «фут» (замір на проді 16.09.2026, 49 474 рядки).
 *
 * ТОЙ САМИЙ ПОРІГ СТОЇТЬ У SQL (`length(btrim(t)) >= 3` у search_supplier_pool
 * і list_supplier_products), і дублювання навмисне: SQL боронить базу від
 * будь-якого викликача, а константа не дає запиту вилетіти взагалі.
 *
 * ⚠️ ЕКРАНИ, ЩО ЦИМ КОРИСТУЮТЬСЯ, МУСЯТЬ СКАЗАТИ ПРО ПОРІГ УГОЛОС. Мовчазна
 * порожнеча на двох літерах читається як відповідь про ТОВАРИ («нічого не
 * знайшлось»), хоч насправді це відповідь про ЗАПИТ — його ще не поставили
 * (Артем, 16.09.2026). Тому поруч із кожним таким полем стоїть
 * `SUPPLIER_SEARCH_MIN_HINT`.
 */
export const SUPPLIER_SEARCH_MIN_TERM = 3;

/** Підпис під полем, поки слово коротше за поріг. Один на всі екрани пошуку. */
export const SUPPLIER_SEARCH_MIN_HINT =
  "Введіть щонайменше три літери — шукатиму в товарах постачальників.";
