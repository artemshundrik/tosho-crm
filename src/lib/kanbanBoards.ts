...
export const KANBAN_BOARDS: Record<KanbanBoardKey, KanbanBoardPlan> = {
  ...
  devRequests: {
    /*
     * «Викочено» лишається колонкою — і це рішення переглядалось двічі за один
     * день. 27.08 його з дошки прибрали: 103 картки зі 168 рішень не
     * потребують, а місце займали. Але дошка без останньої колонки перестала
     * читатись як конвеєр: етап, на якому робота ЗАКІНЧУЄТЬСЯ, зник, і рух
     * зліва направо втратив кінець.
     *
     * Тому обидва входи лишаються, і вони не дублюються: колонка відповідає на
     * «чим закінчився шлях цієї картки», а «Щоденник» — на «що ми зробили за
     * вівторок». Друге питання щоденне, тому «Щоденник» окремою вкладкою; але
     * забирати відповідь на перше було зайвим.
     */
