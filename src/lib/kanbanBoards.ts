 ...
 *
 * ЯК ЦЕ ВИРІШЕНО. `offBoard` — стани, які на дошці стовпчика не мають. Даних це
 * не чіпає: жодного `archived_at`, жодного крона — картка лишається в тому
 * самому статусі, і повернути її можна дією. Розглянуті й відхилені: «колонка з
 ...
 *
 * ЯК ДО НИХ ДІСТАТИСЬ. Через фільтр статусів, який на сторінці й так стоїть:
 * обрав «Скасовано» — замість дошки бачиш список. ОКРЕМОЇ КНОПКИ ПІД ЦЕ В
 * ТУЛБАРІ НЕМАЄ І НЕ ТРЕБА. Перша спроба (24.08.2026) поставила туди перемикач
 * «Дошка / Скасовані», і він був поганий одразу з трьох боків: займав постійне
 * місце на екрані заради дії раз на рік, дублював фільтр, який стоїть поруч, і
 * на кожне натискання смикав той самий фільтр — тобто перезавантажував дошку.
 * Виняток — «Запити на доробку»: там перемикач є, бо станів поза дошкою два
 * («Ідеї» та «Не робимо») і фільтра статусів на тій сторінці немає.
 *
...
export const KANBAN_BOARDS: Record<KanbanBoardKey, KanbanBoardPlan> = {
  ...
  devRequests: {
    /*
     * «Викочено» пішло з дошки 27.08.2026 — у нього тепер власний вигляд
     * «Щоденник» (DevRequestLog.tsx). Замір: зі 168 карток 103 були викочені,
     * тобто дві третини дошки не потребували жодного рішення, а місце й увагу
     * забирали щодня. Колонка тут лишила б порожній стовпчик назавжди.
     *
     * Історія від цього не зникає: у «Щоденнику» вона вся, за днями, з
     * кнопкою «У чат» на кожному рядку. А картка з живим хвостом на дошку не
     * перестає потрапляти — гейт релізу тримає її в «Готово локально» з
     * підписом «частина в проді», скільки б шматків уже не поїхало.
     */
    onBoard: ["triage", "queued", "in_progress", "done_local"],
    offBoard: ["someday", "wont_do", "released"],
  },
};
