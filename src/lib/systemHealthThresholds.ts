...
/**
 * Чи дотягнуть кредити хостингу до кінця біллінгового циклу.
 *
 * ЧЕРВОНЕ — коли не дотягує навіть фон, тобто те, що витрачається БЕЗ жодного
 * викочування (функції, запити, трафік). Тут уже нічого не вирішує «пушити
 * рідше»: треба або докуповувати, або зменшувати фон. І ціна бездіяльності
 * висока — коли баланс дійде нуля, Netlify не «забороняє деплой», а зупиняє
 * сайти, а автопоповнення в нас вимкнене навмисно.
 *
 * ЖОВТЕ — коли не дотягує поточний темп разом із викочуваннями. Це ще
 * керовано: достатньо возити пачками рідше.
 *
 * СІРЕ (не зелене) для норми — навмисно. Це не перевірка, яка «пройшла», а
 * число, яке власник просив бачити щодня; сірі сигнали друкуються окремим
 * рядком завжди, тоді як зелені злипаються в «решта в нормі».
 */
export function classifyNetlifyCredits(params: {
  /** Пакет плюс докуплений запас. */
  creditsLeft: number;
  /** Витрата за добу без викочувань. */
  backgroundPerDay: number;
  /** Витрата за добу разом із викочуваннями. */
  burnPerDay: number;
  daysLeft: number;
}): HealthTone {
  const { creditsLeft, backgroundPerDay, burnPerDay, daysLeft } = params;
  if (daysLeft <= 0) return "neutral";
  if (backgroundPerDay * daysLeft > creditsLeft) return "danger";
  if (burnPerDay * daysLeft > creditsLeft) return "warning";
  return "neutral";
}
