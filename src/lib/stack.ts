 ...
 *
 * НАВІЩО. Питання «чи вже можна взяти мажор N» зводиться до однієї речі — де
 * стоїть ВЕРХНЯ межа чужого peer-діапазону. Нижня не обходить: ми зазвичай вище.
 *
 * ПЕРШИЙ СПОЖИВАЧ ЦЬОГО ВЖЕ НЕ ПОТРІБЕН. Помічник писався під одну умову —
 * `typescript-eslint` тримав `>=4.8.4 <6.1.0` і не пускав TypeScript 7. REQ-208
 * прибрав не межу, а сам typescript-eslint: лінт переїхав на oxlint, єдиній
 * перевірці, що лишилась за ESLint, вистачило `@babel/eslint-parser`, і сімка
 * стала звичайною залежністю. Сторож за цією умовою знятий разом із нею.
 *
 * Функція лишена навмисно, з тестами: наступна чужа умова, яка нас
 * заблокує, читатиметься тим самим способом.
 *
