...
export async function searchSupplierPool(
  ...
  // дістається КОЖНОМУ (на 10.09.2026 це сім джерел × 200). Груп вистачає —
  // «Футболка SoftStyle 153» у totobi це 55 рядків, найтовща з відомих.
  //
  // І НЕ МЕНШЕ ДВОХСОТ, скільки б карток не просили. Вікно візарда — шість
  // карток, тобто за множником лише 30 рядків на джерело, і на такій вибірці
  // злиття за артикулом майже не спрацьовує: заміряно 08.09.2026 — «футболка»
  // нуль злитих карток із восьми, «поло» нуль із восьми, «star» нуль із шести.
  // На двохстах рядках ті самі запити дають 4, 6 і 6. Причина не в кількості
  // карток — злиття йде ДО ліміту, — а в тому, що партнер просто не приїхав:
  // квота ріже рядки Аванпринта за абеткою раніше, ніж дійде до потрібного.
  //
  // ЦІНА ПИТАННЯ РОСТЕ З КОЖНИМ ДЖЕРЕЛОМ, І ЦЕ ГОЛОВНЕ, ЩО ТУТ ТРЕБА ЗНАТИ.
  // Квота ПОСТАЧАЛЬНИЦЬКА, тож вага відповіді — це 200 рядків × скільки джерел
  // у пошуку, а не стеля на всіх. Перший замір (08.09.2026, три джерела) дав
  // 175 кБ, і цифра простояла тут як «нормально» ще два тижні, поки джерел
  // ставало сім. Перезаміряно на проді 10.09.2026, запит «футболка»: збігів
  // 6006, у вікно лягає 860 рядків, відповідь — 342 кБ. Тобто рівно вдвічі
  // більше, і подвоїлось воно мовчки.
  //
  // ЩО З ЦИМ РОБИТИ, КОЛИ ДЖЕРЕЛ СТАНЕ ДЕСЯТЬ. Не зменшувати двохсотку —
  // причина, з якої вона тут, нікуди не ділась. Різати треба інакше: не
  // віддавати рядки джерел, у яких для цього запиту немає жодної своєї ціни,
  // або добирати другу сотню лише коли перша не дала злитих карток. Але це вже
  // не константа, а зміна форми RPC.
  //
  // Тримає вагу в межах те, що запит іде не на кожну клавішу: debounce 250–300
  // мс і `staleTime: 60_000` у обох викликачів.
  const perSupplier = Math.max((options.limit ?? 40) * 5, 200);
