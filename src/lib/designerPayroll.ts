...
export async function loadAbsences(params: {
  ...
  const { data, error } = await supabase
    ...
    .eq("status", "approved")
    // «З дому» — присутність, а не відсутність: людина ПРАЦЮЄ, тож норма
    // не ріжеться. Без цього фільтра тиждень wfh зменшував би норму і
    // ПІДВИЩУВАВ бонус за перевиконання — той самий фінансовий важіль, який
    // закривають межі лікарняних. Тест стереже зняття фільтра.
    .neq("kind", "wfh")
...
export async function loadNormPlans(params: {
  ...
  const [defaults, rates, exceptionRows, absenceRows] = await Promise.all([
    ...
    supabase
      ...
      // і «з дому» серед них немає — людина працює.
      .eq("status", "approved")
      .neq("kind", "wfh")
