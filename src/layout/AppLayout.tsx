...
import React, { Suspense, lazy, useEffect, useLayoutEffect, useMemo, useRef, useState, type ReactNode } from "react";
...
import { pageChromeStyle, usePageChrome } from "@/layout/pageChrome";
...
const baseSidebarLinks: SidebarLink[] = [
  ...
  // Акаунт
  //
  // Заміри 2026-08-05: власник бачив 17 пунктів, CEO — 14, решта — 8. При
   ...
   *
   * Її бачать двоє (власник і CEO), і це кухня самої CRM, а не робота компанії,
...
function AppLayoutInner({ children }: AppLayoutProps) {
   ...
   */
  // Стан верхньої обв'язки при прокрутці — правила в layout/pageChrome.ts.
  ...
  const visibleSidebarLinks = useMemo(
    () =>
      sidebarLinks.filter((link) => {
        ...
        // Observability. CEO має Dev, але не має цієї сторінки, і показувати
      ...
      >
        ...
        <header
          ...
          className={cn(
            ...
            // Шапка не ховається, z-30 — див. layout/pageChrome.ts.
        ...
        <main
          className={cn(
            ...
            isCanvasMode
              ...
              /*
               * 76px = 57 під шапку + 19 повітря. Відколи смуга дій ЛИПНЕ
               * (top = висота шапки), ці 19 стали видимою вадою: на сторінках
               * зі смугою вона при першій же прокрутці підстрибувала в цю
               * щілину й поводилась інакше, ніж на решті. Тому там резерв
               * рівно під шапку, а повітря дає САМА смуга своїм верхнім
               * відступом (PageHeaderActionsSlot) — і воно їде разом із нею.
               *
               * Де смуги немає, 76px лишаються: там ці 19 — єдине повітря між
               * шапкою й першим рядком сторінки. Перша спроба зрівняла обидва
               * випадки, і на «Огляді» привітання втиснулось просто в шапку.
               */
              : hasToolbarAboveContent
                ? "pt-[calc(var(--app-header-base-height)+var(--view-as-offset,0px))]"
        ...
        >
          ...
          */}
          ...
          <div
            ...
            // Скільки верху зайнято обв'язкою — для липких шапок таблиць.
            style={pageChromeStyle(chrome)}
          >
            <div
              className={cn(
                isCanvasMode
