...
/**
 * Лічильник на дзвіночку. Тест тримає три числа, які було зламано по черзі:
 * розмір (20 px на кнопці 40 px), кріплення (відсотковий зсув, що залежав
 * від кількості цифр) і, найновіше, ЯКОРУ (фіксований відступ від кута
 * КНОПКИ — токен `--control-h` сам стискається на брейкпоінті md, а іконка
 * лишається 16px, тож той самий відступ на меншій кнопці зʼїдав центр
 * дзвіночка). Бейдж тепер кріпиться до обгортки навколо ІКОНКИ, а не до
 * кнопки.
 */
describe("лічильник непрочитаних", () => {
  it("кріпиться фіксованим відступом від іконки, а не відсотковим зсувом чи кутом кнопки", () => {
    renderMenu({ unreadCount: 3 });
    const badge = screen.getByText("3");
    expect(badge.className).toContain("-right-2.5");
    expect(badge.className).toContain("-top-2.5");
    expect(badge.className).not.toContain("translate-x-1/3");
    expect(badge.className).not.toContain("-translate-y-1/3");
    // Старе кріплення до кута КНОПКИ (без мінуса) — саме воно ламалось, коли
    // --control-h стискався на md.
    expect(badge.className).not.toContain("right-1 ");
    expect(badge.className).not.toContain("top-1 ");
    // Кільце кольору тла відділяє бейдж від глифа замість голого накладення.
