...
/**
 * «ХТО ЗАРАЗ У ЦІЙ КАРТЦІ» — НАКЛАДКОЮ, А НЕ РЯДКОМ (REQ-164).
 *
 * ЧОМУ НЕ В ПОТОЦІ. Перший захід ставив аватарки звичайним елементом: на
 * прорахунках — поруч із номером, на дизайні — окремим рядком над назвою. Обидва
 * рази вони змінювали висоту картки, і дошка сіпалась щоразу, коли хтось
 * заходив у картку або виходив з неї. Причина проста: аватарка на 18px вища за
 * рядок тексту на 12px, тож рядок росте разом із нею.
 *
 * Накладка з `absolute` не бере участі в потоці взагалі — висота картки не
 * змінюється НІКОЛИ, хоч би скільки людей туди зайшло. Заразом зникає друга
 * проблема: два різні місця на двох дошках. Тепер воно одне.
 *
 * КУТОК ЛІВИЙ ВЕРХНІЙ, ЧАСТКОВО ЗА МЕЖЕЮ КАРТКИ. Правий верхній зайнятий на
 * обох дошках — там мітки типу й меню «⋯». Лівий вільний, а виступ за межу
 * читається як «приліпили зверху», тобто як щось тимчасове, — чим воно і є.
 */
export function CardViewersBadge({ viewers }: { viewers: WorkspacePresenceEntry[] }) {
  if (viewers.length === 0) return null;
  return (
    <span
      className="absolute -left-1.5 -top-1.5 z-10 inline-flex rounded-full bg-background p-0.5 shadow-sm"
      title={`Зараз у картці: ${viewers.map((viewer) => viewer.displayName).join(", ")}`}
    >
      <PresenceAvatarStack entries={viewers} max={3} size={18} tight />
    </span>
  );
}
