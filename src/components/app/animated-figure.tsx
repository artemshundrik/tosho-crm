...
/**
 * Смуга розкривається ЗАТИНАННЯМ, а не зростанням часток. Пояснення важливе,
 * бо очевидний спосіб тут не працює взагалі.
 *
 * ЩО НЕ СПРАЦЮВАЛО: `flex-grow` з нуля до справжніх ваг. Виглядало так, ніби
 * смуга не анімується зовсім, — і це не збій анімації, а сама природа
 * `flex-grow`. Він ділить ВІЛЬНЕ МІСЦЕ пропорційно до ваг: поки всі ваги нулі,
 * місце не ділиться й смуга порожня, але щойно ваги стають хоч трохи більші за
 * нуль, вільне місце роздається ЦІЛКОМ. А оскільки всі частки їдуть одним
 * переходом, їхнє співвідношення на кожному кадрі однакове — тобто смуга
 * стрибає з порожньої на повну за один кадр і далі стоїть.
 *
 * ЩО ПРАЦЮЄ: частки одразу мають справжні ваги, а видно спершу нічого —
 * `clip-path` відрізає смугу справа наліво й від'їжджає. Читається як
 * «смуга наливається», масштаб нічого не спотворює (на відміну від `scaleX`,
 * який розтягнув би заокруглення), і рахує це композитор, а не розкладка.
 *
 * ЗМІНА ЗНАЧЕНЬ — ОКРЕМА РІЧ. Перехід між двома НЕнульовими станами (перемкнули
 * місяць) `flex-grow` робить чудово: обидва стани ділять усе місце, міняються
 * лише пропорції, і частки їдуть одна відносно одної. Тому він лишається — але
 * лише для цього.
 */
export function figureBarRevealStyle(ready: boolean): React.CSSProperties {
  return {
    clipPath: ready ? "inset(0 0 0 0)" : "inset(0 100% 0 0)",
    transition: `clip-path ${FIGURE_REVEAL_MS}ms ${FIGURE_REVEAL_EASING}`,
  };
}

/** Перехід самих часток — на зміну значень, не на появу. */
export const FIGURE_BAR_SEGMENT_TRANSITION = `flex-grow ${FIGURE_REVEAL_MS}ms ${FIGURE_REVEAL_EASING}`;
