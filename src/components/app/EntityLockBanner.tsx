...
type EntityLockBannerProps = {
  ...
  subject: string;
  /** Власник і SEO — резервний сценарій, коли людина недоступна. */
...
export function EntityLockBanner({
    ...
    return (
      <div className={cn(SHELL, CALM, className)}>
        <div className="flex flex-wrap items-center gap-3">
          <TimerReset className="h-4 w-4 shrink-0 text-muted-foreground" aria-hidden />
          ...
          <Button size="sm" variant="outline" onClick={lock.resume} className="shrink-0">
      ...
      <div className={cn(SHELL, "tone-warning", className)}>
        <div className="flex flex-wrap items-center gap-3">
          ...
          <span className="min-w-0 flex-1">
            ...
            <span className="font-medium tabular-nums">{lock.idleSecondsLeft} с</span> через
            відсутність активності.
            {lock.releaseRequestedByName ? ` ${lock.releaseRequestedByName} чекає на доступ.` : ""}
          </span>
          <Button size="sm" onClick={lock.keepAlive} className="shrink-0">
            Я тут
          </Button>
        </div>
      </div>
  ...
  if (lock.acquired && lock.releaseRequestedByName) {
    return (
      <div className={cn(SHELL, CALM, className)}>
        <div className="flex flex-wrap items-center gap-3">
          <AvatarBase name={lock.releaseRequestedByName} size={26} showStatusIndicator={false} />
          <span className="min-w-0 flex-1">
          ...
          {onRelease ? (
            <Button size="sm" onClick={onRelease} className="shrink-0">
              Звільнити
            </Button>
          ) : null}
        </div>
  ...
  if (lock.lockedByOther) {
    ...
    return (
      <div className={cn(SHELL, CALM, className)}>
        <div className="flex flex-wrap items-center gap-3">
          <AvatarBase name={holder} size={26} showStatusIndicator={false} />
          <LiveDot />
          <span className="min-w-0 flex-1">
