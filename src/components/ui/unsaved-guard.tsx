...
export function useUnsavedGuard({ enabled, isDirty }: UnsavedGuardOptions) {
  ...
  return {
    /**
     * Слухач подій. Його ОБОВ'ЯЗКОВО треба рендерити ВСЕРЕДИНІ вмісту модалки —
     * тобто там, де він з'являється разом із відкриттям і зникає при закритті.
     *
     ...
     */
    Listener: () => <UnsavedGuardListener enabled={enabled && auto} touchedRef={touchedRef} />,
    /**
