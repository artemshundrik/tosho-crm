...
import { Check, ChevronDown, ChevronUp } from "@/components/icons/appIcons"
...
/**
 * Та сама шкала, що в Input: розмір задає лише висоту, радіус — роль «контрол»
 * із CONTROL_BASE (REQ-271#p8). Типовий розмір — `md` (32 / 40 на телефоні).
 */
const SELECT_TRIGGER_SIZE = {
  sm: "h-(--control-h-sm) px-2.5 py-1 text-xs",
  md: "h-(--control-h) px-3 py-1.5 text-sm",
  lg: "h-10 px-3 py-1.5 text-sm",
} as const
...
>(({ className, children, controlSize = "md", active = false, ...props }, ref) => (
...
>(({ className, children, position = "popper", portalled = true, ...props }, ref) => {
  const content = (
    <SelectPrimitive.Content
      ref={ref}
      className={cn(
        "relative z-50 min-w-[8rem] overflow-hidden rounded-xl border border-border/50 bg-popover/95 backdrop-blur-xl text-popover-foreground shadow-menu",
...
>(({ className, children, description, ...props }, ref) => (
  <SelectPrimitive.Item
    ref={ref}
    className={cn(
      "relative flex w-full cursor-pointer select-none flex-col items-stretch rounded-lg py-1.5 pl-2 pr-8 text-sm leading-[20px] outline-none transition-colors focus:bg-muted focus:text-foreground data-[state=checked]:bg-muted data-[state=checked]:text-foreground data-[disabled]:pointer-events-none data-[disabled]:opacity-50",
      className
    )}
    {...props}
  >
