  ...
  build: {
    rollupOptions: {
      output: {
         ...
         *
         * Заміряно на проді 21.08.2026, холодне відкриття картки прорахунку:
         * 86 запитів за скриптами, з них ~80 — файли по 0–2 кБ. Сама сторінка
         * приїжджала за 108 мс, а черга дрібниць тягнулась 1.8 секунди й
         * тримала дані сторінки до 3.8 с. У збірці таких файлів менших за
         * 3 кБ було 102 штуки — на 51 кБ сумарно. Тобто сто запитів заради
         * обсягу однієї картинки.
         *
         * Rollup сам не знає, що для нас дорожчий запит, а не байт: за
         * замовчуванням він ріже якнайдрібніше заради кешування. Ставимо поріг
         * і дозволяємо йому склеювати.
         */
      },
    },
  },
