...
export type SupplierPoolMatch = {
  ...
  name: string;
  /**
   * Артикул — лише коли за адресою стоїть ОДИН товар. Поки кольорів кілька,
   * поле мовчить: артикул це те, чим замовляють, і взятий навмання він тихо
   * привезе не той колір. Приїде разом із вибором кольору.
   */
  article: string | null;
  /** Колір ще не обрано — за адресою кілька варіантів. */
...
export function pickPoolMatch(rows: PoolQueryRow[], matchedBy: SupplierPoolMatch["matchedBy"]): SupplierPoolMatch | null {
  ...
  /**
   * КОЛІР НЕ ОБРАНО — АРТИКУЛА НЕМАЄ, і ознака тут саме «варіантів більше за
   * один», а НЕ «артикули різні».
   *
   * На цьому я помилився двічі. У Е-Сувеніра кольори мають різні артикули, і
   * правило «різні артикули» там спрацьовувало. А в Топтайма артикул один на
   * всі чотирнадцять кольорів (`ST7000`) — різняться фото й ціна
   * (269,99–301,35). Тобто за посиланням картка мовчки брала перший колір, і
   * гірше: артикул їхав у позицію, а він ховає й самі плитки вибору
   * (`ImportDraftRow` показує їх лише поки `sku` порожній).
   *
   * Тепер так само, як у пошуку за назвою: `groupSupplierPoolRows` теж лишає
   * картку без артикула, поки варіантів більше одного. Один рядок за адресою —
   * вибирати нема з чого, артикул віддаємо одразу.
   */
  const ambiguousArticle = mine.length > 1;
