...
const SIGNAL_EXPLANATIONS: Record<string, { what: string; why: string; todo: string }> = {
  ...
  netlify_credits: {
    what: "Скільки лишилось кредитів Netlify — це оплачений пакет хостингу, з якого списується кожне викочування (15 кредитів) і робота сайту: функції, запити, трафік.",
    why: "Пакет оновлюється раз на місяць 27 числа. Жовтий означає, що за поточним темпом він скінчиться раніше; червоний — що не дотягує навіть фон, тобто витрата без жодного викочування.",
    todo: "Жовтий лікується темпом: возити коміти пачками рідше. Червоний — докупити кредити ($5 за 500) або зменшувати фон. Доводити до нуля не можна: Netlify тоді зупиняє сайти, а автопоповнення в нас вимкнене.",
  },
  attachments: {
