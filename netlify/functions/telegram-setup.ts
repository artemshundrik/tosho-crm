...
//
// «task», а не «задача»: Bot API приймає в списку команд ЛИШЕ [a-z0-9_], тож
// кирилиця валить увесь виклик setMyCommands. Основна «/задача» при цьому
// повністю робоча — Telegram доставляє незнайому команду звичайним текстом, і
// вебхук розбирає обидві (_lib/devRequestBot.ts).
const COMMANDS = [
  { command: "menu", description: "Швидкі дії" },
  { command: "task", description: "Записати задачу (або просто перешли повідомлення)" },
  // «queue», а не «черга» — з тієї ж причини, що «task»: у списку команд
  // кирилиця заборонена. Обидві форми працюють, у меню видно латинську.
  //
...
const DESCRIPTION =
  ...
  "Щось не працює або хочеться зручніше — перешли мені повідомлення чи напиши /задача: зроблю картку в CRM.";
