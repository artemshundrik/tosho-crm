...
- **Avatar (`uploadAvatarBlob`):** file → `react-easy-crop` → three WebP variants (`xs/md/hero`) uploaded to the `avatars` bucket under `avatars/{userId}/{ts}/`. Кадрування, розміри й заливка живуть у спільному `src/features/team/avatarUpload.ts` — тим самим модулем керівник міняє фото в картці людини (`AvatarCropDialog`); тут лишилась ЛИШЕ своя частина — writes `avatar_path` to both `team_member_profiles` and auth metadata, refreshes session, removes stale variant files, dispatches `profile:avatar-updated`. Rendered via `AvatarBase` (avatar-kit) — never a raw `<img>`.
