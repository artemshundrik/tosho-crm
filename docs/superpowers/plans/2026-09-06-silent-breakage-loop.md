...
Рядок підключення — у змінній оточення `BACKUP_DB_URL`; підставляй її як
`"$BACKUP_DB_URL"` і **ніколи не друкуй значення** ні у виводі, ні у звіті.
...
### Task 7: Замір вартості прев'ю й перепис політики

Цей блок **потребує дії Артема** й **може коштувати 15 кредитів**. Не починати без прямої команди.

**Files:**
- Modify: `netlify.toml` (перший рядок `ignore`)
- Modify: `docs/DEPLOY_POLICY.md` (§5.3 і §3.3)

- [ ] **Step 1: Артем вмикає branch deploys**

Netlify → Site → Build & deploy → Branches → дозволити `preview/*`.

Агент у налаштування акаунта не лізе. Поки не підтверджено — далі не йти.

- [ ] **Step 2: Обмежити пропуск збірки до deploy-preview**

У `netlify.toml`, перший рядок блоку `ignore`, замінити:
...
Так `branch-deploy` почне збиратись, а PR-и від Dependabot і далі пропускатимуться.

- [ ] **Step 3: Запушити гілку заміру**

**Тільки за прямою командою Артема** — це пуш.

```bash
git switch -c preview/netlify-cost
git commit --allow-empty -m "Замір: скільки коштує branch-деплой"
git push -u origin preview/netlify-cost
```

- [ ] **Step 4: Прочитати рахунок**

Netlify → Team → Billing → Credit usage breakdown. Записати число до й після.

Expected: або 0 кредитів (документація Netlify каже так), або 15.

- [ ] **Step 5: Переписати §5.3 під замір**

Зараз там стоїть «перевірено 29.08.2026», хоча §2 у тому ж файлі зізнається «за документацією Netlify». Замінити на справжній результат із датою й числом із Credit usage breakdown.

- [ ] **Step 6: Переписати §3.3**

Зараз: «Перевірка перед викочуванням — це `npm run dev` на localhost».

Має стати — три різні відповіді на три різні потреби:

- заміри продуктивності — `npm run build && npm run preview`, бо dev завищує приблизно вдесятеро;
- мовчазні поломки (виклики до бази, адреси кронів, RLS) — ратчети в `npm run check`, а не людське око;
- показати зміну людині за посиланням — прев'ю, єдиний вжиток, який нічим не замінюється.

- [ ] **Step 7: Прибрати гілку заміру**

```bash
git switch main
git push origin --delete preview/netlify-cost
git branch -D preview/netlify-cost
```

- [ ] **Step 8: Коміт**

```bash
git add netlify.toml docs/DEPLOY_POLICY.md
git commit -F - <<'EOF'
Правила деплою тепер спираються на замір, а не на документацію постачальника

§5.3 стверджував «перевірено», хоча §2 у тому ж файлі зізнавався, що число
взяте з документації Netlify. Тепер там результат справжнього branch-деплою
з датою.

§3.3 більше не відповідає «дивись локально» на три різні питання: заміри —
через зібраний застосунок, мовчазні поломки — ратчетами, показ людині —
прев'ю.

Закриває: REQ-103#p1, REQ-103#p2, REQ-103#p3, REQ-103#p4
...
- [ ] **Step 3: Взяти число витрат із CRM**

Витрати Netlify з сесії не читаються — `NETLIFY_API_TOKEN` живе в оточенні Netlify. Спитати бота інтентом `hosting_usage` (`netlify/functions/_adminAssistant.ts`) і взяти число звідти.

Порівняти з ~13 кредитами на добу до зміни.

- [ ] **Step 4: Закрити пункти**

```bash
git commit --allow-empty -F - <<'EOF'
Нагадування прокидаються раз на п'ять хвилин — підтверджено на проді

Крон стоїть на */5, за добу 288 прогонів замість 1440. Витрати звірено з
рахунком через бота.

Закриває: REQ-217#p1, REQ-217#p2
...
Task 7 (замір прев'ю)   [окремо, потребує дії Артема й може коштувати 15 кредитів]
...
| Task 7 | Число з Credit usage breakdown у тексті §5.3 |
