...
- **Supabase** — читання робочої бази без psql: схема, дані, логи, поради. Підключений як віддалений HTTP-сервер із параметрами прямо в URL (`https://mcp.supabase.com/mcp?project_ref=…&read_only=true&features=database,debugging,docs`), авторизація — OAuth через браузер, тож токена немає ні в конфігах, ні в репозиторії. Це навмисно замість локального `npx @supabase/mcp-server-supabase`: тому потрібен особистий токен `SUPABASE_ACCESS_TOKEN` у налаштуваннях, а це секрет на диску. `read_only=true` ріже набір до восьми інструментів (`search_docs`, `list_tables`, `list_extensions`, `list_migrations`, `execute_sql`, `get_logs`, `query_logs`, `get_advisors`) — `apply_migration` у ньому немає, і це перевірено: без прапорця він у наборі з'являється. Тобто МІНЯТИ схему через MCP не можна за побудовою; міграції й далі їдуть через `npm run db:apply`.

Коли брати Supabase MCP — інакше він лежить без діла, бо звичка вгадувати з коду сильніша:

| Ситуація | Замість здогадки |
|---|---|
| «які там колонки, які значення реально бувають» | `list_tables`, `execute_sql` — `docs/DB_MAP.md` оновлюється руками й відстає |
| «скільки разів це вживали» перед тим, як прибрати чи перебудувати | `execute_sql` — саме такі заміри вирішили порядок у QUOTE_KINDS і долю категорії «Інше» |
| «на проді не працює, а локально працює» | `get_logs`, `query_logs` (Postgres, PostgREST, Auth, Edge) |
| зміна RLS, політик або доступів — перед «готово» | `get_advisors`, поруч із [docs/SECURITY.md](/Users/artem/Projects/tosho-crm/docs/SECURITY.md) |
| «чи поїхала міграція на прод» | `list_migrations` |
| потрібен живий приклад даних, щоб перевірити поведінку в браузері | `execute_sql`, далі очима в прев'ю |

Межа, про яку легко забути: репозиторій публічний, тож у коміти, доки й звіти йдуть числа й форма даних — не вміст. Імена, контакти й суми конкретних замовників лишаються у виводі запиту.

Ціна трьох серверів у контексті (заміряно 05.09.2026 через `tools/list`, символи готового JSON поділені на 4): Chrome DevTools — 29 інструментів ≈ 5 700 токенів, Supabase — 8 ≈ 1 300, Context7 — 2 ≈ 1 200. Разом 39 інструментів ≈ 8 200 токенів у кожному запиті сесії.

Двох кандидатів за результатом цього заміру ВІДХИЛЕНО, повертатись без нових аргументів не варто:

- **Netlify MCP** — 9 інструментів ≈ 2 900 токенів, з них половина `-updater`: вони деплоять і міняють змінні оточення, тобто дають моделі кнопку, яку політика деплою ([docs/DEPLOY_POLICY.md](/Users/artem/Projects/tosho-crm/docs/DEPLOY_POLICY.md)) лишає за людиною. Читацька половина дублює `npx netlify`, який і так дозволений.
- **Postgres MCP Pro** — вимагає рядок підключення до прод-бази з паролем прямо в конфізі MCP, тоді як Supabase MCP ходить по OAuth і лише на читання. Його інструменти перекриваються з `features=database,debugging`, а для міграцій у проєкті вже є `npm run db:apply` і psql у транзакції з `rollback`.
...
## 16A. Наскрізні перевірки в браузері (Playwright)

Модульні тести перевіряють код; ці перевірки — застосунок так, як ним
користується людина. Приводом стало те, що при зелених 1217 тестах, `tsc` і
лінті в прод поїхали блимання сторінки прорахунків при закритті вікна хрестиком
і кнопка «Скасувати», яка перестала закривати форму (REQ-140).

Запуск:

```
npm run e2e         # зібрати застосунок і прогнати сценарії
npm run e2e:report  # відкрити звіт останнього прогону
npm run e2e:login   # вхід у справжньому вікні — лише коли треба подивитись під живою людиною
```

`E2E_SKIP_BUILD=1` пропускає збірку, коли `dist` уже свіжий, — для налагодження
самих сценаріїв.

**Перевірка очима йде під службовим акаунтом, а не під твоєю сесією.** З
11.09.2026 у проді живе «Тиць Клікович» (`e2e@tosho.pro`, REQ-147): посада СЕО,
щоб жодна сторінка не впиралась у «немає доступу», і аватарка-робот, щоб у
«Команді» було видно, що це не людина. `E2E_EMAIL` / `E2E_PASSWORD` лежать у
`.env.local`; `e2e/globalSetup.ts` заходить ними сам, тож `npm run e2e`
працює без жодної підготовки. Заводить акаунт `node scripts/e2e-account-setup.mjs`
— повторний запуск безпечний, `--reset-password` видає новий пароль.

Що про нього треба пам'ятати:

- **Прав у нього як у СЕО, і від записів його тримає лише `writeGuard`.** Поза
  прогоном це звичайний повноправний акаунт у проді. Тому пароль живе тільки в
  `.env.local` та секретах GitHub, а сторожа не «вимикають на хвилинку»: для
  записів є окрема сесія під живою людиною.
- **У зарплатну відомість він не потрапляє** — його user_id у
  `PAYROLL_EXCLUDED_USER_IDS` (`src/lib/payroll.ts`). Решту списків людей
  («Команда», згадки, присутність) він поповнює як звичайний рядок; це
  свідомий вибір, щоб не правити всіх читачів заради тестового акаунта.

Що треба знати, перш ніж дописувати сценарій:

- **Записів не буває.** Supabase у проєкті один, продівський. Гальмо в
  `e2e/writeGuard.ts` глушить будь-який запит, що змінює дані, на рівні
  браузера. Спроба запису валить тест — це не «сторож спрацював», а помилка в
  сценарії.
- **Підміняти `window.fetch` зі сторінки марно** — клієнт supabase-js тримає
  власне посилання на нього. Перехоплювати можна лише через `page.route`.
- **Перевірки пишуться на структуру, а не на дані**: «п'ять колонок», «вікно
  закрилось», а не «є картка TS-0826-0034». База жива, і дані змінюються.
- **Сторож стоїть на кожному сценарії сам** (`e2e/fixtures.ts`, `auto: true`):
  помилки консолі, незловлені винятки, невдалі запити й спроби запису. Шум
  описується поіменно в переліку дозволеного, з поясненням, чому він нешкідливий.
- **Пуші не блокуються.** Прогін живе в окремому робочому процесі
  `.github/workflows/e2e.yml` (руками або щоночі), а не в `ci.yml`. У `ci.yml`
  перевіряються лише типи сценаріїв — `npm run typecheck:e2e`.
