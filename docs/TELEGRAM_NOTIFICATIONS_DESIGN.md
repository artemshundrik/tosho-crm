...
> Статус: **Фаза 1 — ЗАДЕПЛОЄНО на prod. Фази 2–3 — код у working tree (НЕ закомічено).** Бот `@ToShoCRM_bot`.
> Фаза 1 (live): SQL-схема застосована; `_telegram.ts`; webhook `/start`+`/stop`; Telegram-канал у `_notificationDelivery.ts`; UI «Підключити Telegram» + тумблер у `ProfilePage.tsx`; env + webhook зареєстровано.
> Фаза 2 (working tree): реєстр категорій `src/lib/notificationCategories.ts` + `netlify/functions/_notificationCategories.ts`; усі продюсери передають `category` у `deliverNotifications`; гейтинг push+telegram за категорією у `_notificationDelivery.ts`; матриця «тип × канал (push/telegram)» у діалозі налаштувань `NotificationsPage.tsx` (пише `channel_prefs`). DDL не потрібен — `channel_prefs` уже існує.
> Фаза 3 (working tree): `telegram-webhook.ts` розширено — `/settings` з інлайн-кнопками + `callback_query` тоглить `channel_prefs`/`telegram_enabled` у тому самому рядку → синхронно з CRM.
> **Фази 2–3 не закомічено й не задеплоєно** — пушимо лише за прямою командою CEO. Після пушу/деплою треба **перереєструвати webhook** з `allowed_updates` `["message","callback_query"]` (див. §12), інакше кнопки в боті не працюватимуть.
>
...
3. **Зареєструвати вебхук** (після деплою функцій). Для фази 3 обов'язково додати `callback_query` в `allowed_updates`, інакше кнопки `/settings` не працюватимуть:
   ```sh
   curl -s "https://api.telegram.org/bot<TOKEN>/setWebhook" \
     --data-urlencode url="https://tosho.pro/.netlify/functions/telegram-webhook" \
...
## 14. Відсутності в боті (серпень 2026)

Бот тепер не лише читає, а й пише — рівно одну річ: **власну відсутність**.

**Команди й швидкі кнопки** (реєструються `telegram-setup`, кнопки — у «📋 Меню»):

- `/away` або питання «хто сьогодні відсутній?» — зведення з журналу
  `tosho.team_absences`: хто відсутній сьогодні, що зміниться завтра, що на
  погодженні. Доступно всій прив'язаній команді (типи відсутностей і так
  видно кожному в планері; коментарі-причини сюди не потрапляють).
- `/absence` — оформлення кнопками: лікарняний (пресети «сьогодні»,
  «сьогодні–завтра», «до кінця тижня», «вчора–сьогодні»), day-off
  («завтра»/«післязавтра»), відпустка (текстом).
- Текстом без команд: «хворію», «лікарняний 05.08–08.08», «day-off 15.08»,
  «відпустка 12.08–19.08». Перехоплюється ДО асистента (не палить модель) і
  лише коли текст ПОЧИНАЄТЬСЯ з ключового слова — «хто у відпустці?» іде
  асистенту як `who_is_absent`.

**Шлях запису.** Вебхук не має JWT користувача, тому вставка йде через
`tosho.bot_submit_absence` (`scripts/team-absences-bot-rpc.sql`) — SECURITY
INVOKER функція, доступна лише service_role, яка перевтілюється в користувача
(`set_config('request.jwt.claims', …)` + `SET LOCAL ROLE authenticated`).
Завдяки цьому RLS-межі дат, «лише за себе» і річна квота лікарняних працюють
точно як у CRM — правила живуть в одному місці. Сповіщення після подання —
спільною лібкою `netlify/functions/_lib/absenceSubmit.ts` (та сама, що й у
HTTP-шляху `team-absence-request`): лікарняний бачить уся команда, заявка на
погодженні — owner/CEO.

**Стан флоу** живе в `callback_data` (`abs:c:sick_leave:2026-08-02:2026-08-03`),
сервер нічого не тримає; кроки редагують одне повідомлення (`editMessageText`).

**Після зміни списку команд** — один раз POST `/.netlify/functions/telegram-setup`
з `x-cron-key`, щоб Telegram оновив меню.
