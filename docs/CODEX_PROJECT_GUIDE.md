...
- **Приватність залишків:** свій баланс бачить кожен, чужі — лише owner/CEO.
...
- Self-service заявки й погодження (CEO вирішує, owner бачить) — Фаза 2,
  через Netlify-функцію за патерном `team-member-probation.ts`.

Індикатор відсутності на аватарці (`src/lib/absenceIndicator.ts`):

- **Два рівні:** `full` (кільце + іконка типу) лише на «Команді»; `simple`
  (кільце) — решта застосунку. Нижче 24px індикатора немає взагалі, нижче
  32px немає іконки — гліф там нечитабельний.
- **Кільце малюється ТІЛЬКИ ring-змінними.** У `src/index.css` є навмисне
  правило, що `!important` прибирає тіні скрізь, окрім Radix-поповерів, —
  власний `box-shadow` мертвий.
- **Кільце живе на обгортці, не на аватарці:** приглушення робиться через
  `grayscale`, а CSS-фільтр знебарвлює і власну тінь елемента.
- **Відсутність перебиває presence** (у відпустці можна зайти з телефону —
  зелена крапка збрехала б), **звільнення перебиває все**.
- Токен accent зветься `--accent-tone-*`, не `--accent-*`: `--accent-foreground`
  це службовий колір тексту, і на ньому легко отримати чорне кільце.
